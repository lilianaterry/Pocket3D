...
class PrintStatusIntentHandler: NSObject, PrintStatusIntentHandling {
    func handle(intent: PrintStatusIntent, completion: @escaping (PrintStatusIntentResponse) -> Void) {
        ...
        API.instance.status { status, filename, percentage, timeRemaining in
            if status == .Ok {
                let filename = String(filename.dropLast(6))
                if percentage == 100 {
                    ...
                    completion(PrintStatusIntentResponse.stillPrinting(filename: filename, completion: percentage, finishTime: dcf.string(from: Double(truncating: timeRemaining))!))
                }
            } else {
